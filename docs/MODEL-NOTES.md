...
- 2026-09-10 — order-journey-build r1 (code-feature ×3): the engine's `model_default = "gpt-5.4"` is DEAD on the ChatGPT-account plan — every worker died at the first API call ("The 'gpt-5.4' model is not supported when using Codex with a ChatGPT account"), 13–35 s, 0 tokens, both attempts. Not a capability signal. Fix was the per-task `"model": "gpt-5.6-terra"` field (the dispatcher's own r-codex runtime already used it); re-run 3/3 PASS. Until config.toml's default is updated, pin `gpt-5.6-terra` per task.
- 2026-09-10 — gpt-5.6-terra (high), code-feature ×3 (journal ~69k, dispatcher-policy ~147k, stall-scan ~125k): dispatcher-policy PASS first try; journal and stall-scan each needed attempt 2 for the same reason — the worker finished the code but did not write `fix-summary.md` (the spec said "in your current directory"; the worktree-mode cwd wording is ambiguous). Product on all three was sound under orchestrator review. Lesson: say "write ./fix-summary.md in the worktree root — the check fails without it" explicitly; budget one retry on terra for the summary.
...
## 2026-09-23 — dashboard-phase1 adversarial review (code-review, 3 reviewers, one 515-line diff)
- GLM 5.2 (opencode): PASS first try, ~127k tokens, ~5.5 min, ~$0.10. Found one real P2 (non-atomic note dedup, no unique key) and confirmed three fixes sound. Missed the P1 Sonnet found. Good cheap second reviewer; not sufficient alone.
- sonnet (claude-readonly): PASS first try, ~5 min. Found the P1 both the Opus builder and GLM missed (connector marker-search path posts Sent with no thread check; new server thread rule rejects it). Strongest reviewer this run.
- gpt-5.4 (codex): FAIL x2 in 10 s — "model is not supported when using Codex with a ChatGPT account". Retired on ChatGPT plans; codex model_default moved to gpt-5.6-luna (Commander, 2026-09-23). Do not route to gpt-5.4.
- gpt-5.6-luna (codex): FAIL x2 — ChatGPT Codex usage limit, resets 2026-09-26 08:51. Not a model failure.
- grok-4.5 (grok): FAIL x2 — 402 "Grok Build usage balance exhausted". Not a model failure; no reset date given.

## Orchestrator note — 2026-09-23 (dashboard-phase2 repo-feature, claude/sonnet ×1)
- sonnet (claude engine, worktree): did the work correctly on attempt 1 (~$0.85, ~11 min); verified by hand at 557/557 on the real checkout. Ringer recorded FAIL x2 — both failures were the orchestrator's check, not the worker:
  1. `git add -A -- . ':!node_modules'` exits 1 when node_modules is a symlink matched by .gitignore; the check crashed and the retry prompt carried a Python traceback. Use `git add -A` without the exclude (ignored paths are skipped) or `git diff HEAD` plus `git ls-files --others --exclude-standard`.
  2. open-brain-web test/slice1-retrieval resolves `../../OB1/...` relative to the checkout; in a worktree under ~/.ringer that path does not exist → 502. Tests with sibling-repo paths cannot pass in Ringer worktrees (see ringer-worktree-check-env-limits); run the suite on the real checkout after applying the patch.
- Lesson: a single-worker repo job gained little from Ringer here; a Sonnet subagent plus the same check run on the real checkout would have been simpler (Commander observation, 2026-09-23).

## sonnet (claude) — 2026-09-23, planner-step3-daily-page / planner-step3-weekly-preview (worktrees, code-feature/code-fix)
- 7 tasks across 4 runs; every task needed attempt 2 (retry injected check output), all substance correct after review. Failures on attempt 1 were mostly my check wrappers (fix-summary section names, node_modules symlink in owned files, a too-literal href grep), not code.
- **Worktree escape:** the week-page worker also wrote draft files (week.html/js/css, an index.html edit) into the MAIN repo checkout via absolute paths, outside its worktree, although the `claude` engine allowlist is `Edit(./**) Write(./**)`. Caught only because a later ff-merge refused to overwrite them. Next time: add `git -C <repo> status --porcelain` must be empty to the check, and never put the main checkout's absolute path in a spec except as read-only background.
- A stale check (required reading a field the page no longer used) made the worker add filler code (`const { rituals } = day; void rituals;`) to satisfy it. Keep checks in step with requirement changes.
- 2026-09-24 — claude-opus-5-5, code-feature (planner-ux-fixes, open-brain-web vanilla JS): 2/2 passed; r1 needed one retry only for fix-summary.md format (spec it in the brief), r2 first try. Neither checked the Vercel route allowlist — new public/plan files 404'd live; allowlist guard now in check_r2.
- 2026-09-24 — claude-opus-5-5: rounds 3-4 (code-fix, code-feature) passed; r4 web retry only; parallel two-repo round (Mac + web) worked with disjoint repos.
